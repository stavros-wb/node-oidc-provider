...
module.exports.handler = function getAuthorizationCodeHandler(provider) {
  ...
  return async function authorizationCodeResponse(ctx, next) {
    ...
    if (!code) {
      ctx.throw(new InvalidGrant('authorization code not found'));
    }

    if (code.isExpired) {
      ctx.throw(new InvalidGrant('authorization code is expired'));
    }
    ...
    if (pkce && (ctx.oidc.params.code_verifier || code.codeChallenge)) {
      try {
        ...
        ctx.throw(new InvalidGrant('PKCE verification failed'));
      }
    }

    try {
      if (code.consumed) {
        ctx.throw(new InvalidGrant('authorization code already consumed'));
      }
    ...
    if (!account) {
      ctx.throw(new InvalidGrant('authorization code invalid (referenced account not found)'));
    }
